...
interface UseSessionSocketReturn {
  ...
  sendPrompt: (content: string, model?: string, reasoningEffort?: string, agent?: string) => void;
  stopExecution: () => void;
  sendTyping: () => void;
  reconnect: () => void;
...
export function useSessionSocket(sessionId: string): UseSessionSocketReturn {
  ...
  const sendPrompt = useCallback(
    (content: string, model?: string, reasoningEffort?: string, agent?: string) => {
      ...
      if (!subscribedRef.current) {
        ...
        setTimeout(() => sendPrompt(content, model, reasoningEffort, agent), 500);
        return;
      }
